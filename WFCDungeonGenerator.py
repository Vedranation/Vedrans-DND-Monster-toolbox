...
class Tile:
    def __init__(
        ...
        self.valid_decorator_locations: List[tuple] = (
            []
        )  # Decorator - items which make map prettier but are not interractible or tangible
    ...
    def _extract_tile(
        ...
        global_grid_x: int,
        global_grid_y: int,
        grid_pixel_size: float = 26,
        blocks_in_tile: int = 12,
    ) -> Image:
        """
    ...
    def DrawRedSquare(self, local_tile_grid_x: int, local_tile_grid_y: int):
        ...
        # draw red square
        top_left = (
            local_tile_grid_x * block_length + int(block_length * 0.25),
            local_tile_grid_y * block_length + int(block_length * 0.25),
        )
...
class TileObjectsFactory:
    ...
    def __init__(self):
        ...
        default_tiles_constructor = [
            ...
            {
                "name": "TileObj_bigroom_walled_entrance",
                "grid_x": x3,
                "grid_y": y3,
                "rmid": 1,
                "ltop": 1,
            ...
            # Y=4
            {
                "name": "TileObj_mini_double_corner",
                "grid_x": x0,
                "grid_y": y4,
                "topl": 1,
                "ltop": 1,
            ...
            {
                "name": "TileObj_bigroom_corner_entrance",
                "grid_x": x2,
                "grid_y": y4,
                "lmid": 1,
                "rtop": 1,
            ...
            {
                "name": "TileObj_bigroom_walled_entrance_side",
                "grid_x": x1,
                "grid_y": y6,
                "rmid": 1,
                "topl": 1,
            ...
            {
                "name": "TileObj_bigroom_corner_entrance_side",
                "grid_x": x3,
                "grid_y": y6,
                "ltop": 1,
                "rtop": 1,
...
def ChooseAndPlaceTile(x: int, y: int) -> list:
            ...
            if not tile_l.tile_type == "empty":  # if tile is empty, no connections are required or blocked
                if tile_l.Connection_points["rtop"]:
                    ...
                    blocked_connections.append(
                        "ltop"
                    )  # If there isn't a connection on occupied tile, we gotta block it
    ...
    for tile in TileObj.default_tiles:
        if all(tile.Connection_points.get(conn_point) == 1 for conn_point in forced_connections) and not any(
            tile.Connection_points.get(conn_point) == 1 for conn_point in blocked_connections
        ):
...
def ThirdPass(tile_dim):
    ...
    n_decorations = 3
    n_obstacles = 2
    grid_pixel_size = screen_size[0] // (
        Grid.width * 12
    )  # Compute DISPLAY size of each pixel (this is different to export picture)
    ...
    data = {
        ...
        "grid_x": 3,
        "grid_y": 11,
    }  # TODO: make it automatic factory in another file
    bear_trap = Decorator(**data)
    data = {"name": "stone_rubble_1", "dimensions": (3, 2), "type": "decorator", "grid_x": 27, "grid_y": 11}
    ...
    for y in range(Grid.height):
        for x in range(Grid.width):
            ...
            global_decorator_y = screen_size[1] - (
                tile_dim * y + (stone_rubble_1.y * grid_pixel_size + grid_pixel_size)
            )
